-- Escola Schmidt — gravação atômica de avaliação (rápida ou detalhada por critério)
-- Rodar em: Supabase > SQL Editor, depois de 0009_minha_evolucao_seed.sql

create or replace function derivar_status_por_percentual_evolucao(p_percentual numeric)
returns status_habilidade_evolucao
language sql
immutable
as $$
  select case
    when p_percentual >= 90 then 'consolidado'::status_habilidade_evolucao
    when p_percentual >= 75 then 'dominado'::status_habilidade_evolucao
    when p_percentual >= 55 then 'em_desenvolvimento'::status_habilidade_evolucao
    when p_percentual >= 20 then 'aprendendo'::status_habilidade_evolucao
    else 'nao_iniciado'::status_habilidade_evolucao
  end;
$$;
