...
**Fechado em 2026-09-20.** `EXPO_PUBLIC_FERIADOS_API_KEY` documentada em `.env.example`. Nova funcao `importarFeriados(uf, ano, criadoPor)` em `feriados.ts` orquestra o fluxo completo: busca via `buscarFeriadosEstado`, resolve o `tipo_id` de "Feriado" via `getTiposEvento`, deduplica contra `getEventosPorPeriodo` do ano (chave `titulo+data`) e cria 1 evento por feriado novo via `criarEvento`. UI: botao "Importar feriados de [ano]" em `eventos/index.tsx` (staff-only, mesma guarda de "+ Evento"), abre card inline com `Dropdown` de UF (27 opcoes) e feedback de quantos foram importados vs. ja existentes. `ConfiguracaoFeriadosError` tratado com mensagem amigavel em vez de erro tecnico. 4 testes novos em `feriados.test.ts` (chave ausente, import limpo, dedup, tipo "Feriado" faltando).
...
3. **Remover secao inline de Desempenho** — **fechado em 2026-09-20.** Removida a grade editavel "Avaliar habilidades" e o formulario de "Registrar passagem de nivel" de `alunos/[id]/index.tsx` (funcoes, estado e estilos mortos removidos junto). A secao "Desempenho" virou "Desempenho (historico legado)": mantem a legenda e a "Linha do tempo" (avaliacoes/testes antigos) **so leitura**, sem editar — preserva o "historico read-only" que a Fase 5 do documento de visao original pede, sem apagar visibilidade dos dados legados.
4. **Repropositar `alunos/[id]/desempenho.tsx`** — **fechado em 2026-09-20.** Vira a tela "Jornada": timeline completa (sem limite de 5) cruzando `historico_nivel_evolucao` (eventos de nivel) com `avaliacoes_evolucao` (avaliacao por habilidade, agora com funcao dedicada `getAvaliacoesEvolucaoAluno` em `api.ts`, ja que so existia snapshot atual antes). Rota religada: atalho novo em `alunos/[id]/index.tsx` (ao lado de "Minha Evolucao") e link "Ver jornada completa" no card "Historico" do `EvolucaoScreen` (que continua limitado a 5 itens, agora com destino) — sem isso a tela ficaria orfa de novo.
5. **Radar chart** — **fechado em 2026-09-20.** Novo componente `src/components/RadarChart.tsx`, desenhado com `react-native-svg` (instalado via `npx expo install react-native-svg`, versao `15.15.4` resolvida pelo instalador do Expo pra SDK 57 — confirmado compativel na documentacao versionada do Expo antes de instalar, por exigencia do `AGENTS.md` do repo) em vez de uma lib de chart pronta — app nao tinha nenhuma dependencia de grafico, e radar de poucos eixos e pouca matematica pra justificar uma lib inteira. Substitui a lista de barras no card "Radar pedagogico" do `EvolucaoScreen` quando ha 3+ categorias (a lista de barras continua abaixo do radar, como leitura precisa complementar). 4 testes novos em `RadarChart.test.tsx`. Verificado rodando o app de verdade (`expo start --web`): boot limpo, zero erros de console, bundle do Metro compilado com sucesso incluindo a dependencia nova.
...
- **Feriados** (`eventos.md` §5.1) — **fechado em 2026-09-20**, ver secao 2.7.
...
- **Aula particular — visibilidade/edicao escopada + autoatendimento do aluno** (`chamada-agenda-frequencia.md` §7.3) — **fechado em 2026-09-20.** Migration `0026_aula_particular_autoatendimento.sql`: `aula_leitura` deixa de liberar `eh_equipe()` geral pra `tipo = 'particular'`, passa a exigir dono/professor da aula/aluno vinculado; `aula_escrita` foi desmembrada em `aula_insercao`/`aula_atualizacao`/`aula_exclusao` (regras diferentes pra insert vs update/delete, que RLS `for all` nao expressa) via helper `sou_dono_da_particular()`; RPC `remarcar_aula_particular` restringe edicao a data/hora, reaproveitando `horarios_livres_particular`. Achado fora do documento original: `perfil_leitura_professor_da_minha_aula` (0006) so deixava aluno ver nome de professor do proprio modulo — sem ajuste, autoatendimento com professor de outro modulo ficaria com o nome invisivel no formulario mesmo a disponibilidade aparecendo certa; nova policy `perfil_leitura_professor_disponivel_particular` abre leitura de nome pra qualquer professor com `disponibilidade_particular` cadastrada. UI: `nova-particular.tsx` (aluno pre-seleciona a si mesmo, sem Dropdown), nova tela `chamada/remarcar-particular.tsx`, `chamada/index.tsx` (botao novo pro aluno, tap/long-press habilitados pra quem a RLS ja deixa ver o item).
- **Aula teste — candidato nao matriculado** (`chamada-agenda-frequencia.md` §7.4) — **fechado em 2026-09-20.** Migration `0027_aula_teste_candidatos.sql`: `aulas_teste_alunos` (FK obrigatoria pra `alunos`) trocada por `aulas_teste_candidatos` (nome, telefone opcional, sem FK) — sem dado de producao a migrar, `drop table` direto. `nova-teste.tsx`: `Dropdown` de matriculados trocado por lista livre de adicionar/remover (mesmo padrao de `disponibilidade.tsx`). `chamada/[id].tsx`: nova secao "Aula Experimental" (nova funcao `getCandidatosTesteDoDia` em `api.ts`), so informativo, sem presenca formal (decisao 3 do documento de origem).
...
## 5.1 Retomada do PO (2026-09-20): catalogo pedagogico e permissoes — item bloqueador implementado

`evolucao-vs-desempenho.md` §7-8 e `papeis-e-permissoes.md` §6-7 trouxeram um novo lote de decisoes depois do fechamento da Fase 4. Seguindo a ordem "bloqueador primeiro" (o proprio §8.3 chama a tela de catalogo de bloqueadora do passo 1 da Fase 4, que ja estava fechado com o fluxo minimo):

- **Migration `0028_categoria_figuras.sql`** — **fechado em 2026-09-20.** Cria a categoria "Figuras" (`categorias_habilidade`) e corrige o dado ja em producao: `habilidades_catalogo` com `slug = 'figuras-alongamento'` sai de "Fundamentos" (onde caiu por um `else` sem tratamento especifico em `0009`) e vai pra "Figuras", confirmando a decisao 7.2 do documento de origem.
- **Tela "Catalogo de Evolucao"** (`app/(app)/catalogo-evolucao.tsx`) — **fechado em 2026-09-20, escopo do achado 8.1.** Dono-only (RLS ja era `papel_atual() = 'dono'` desde `0007`, nenhuma migration de permissao precisou mudar). Tres abas: Categorias (criar + ativar/desativar), Habilidades (criar + ativar/desativar, com campo pro codigo oficial dos regulamentos em `nome_internacional`), Requisitos por nivel (escolher metodologia/nivel, adicionar/remover habilidade com peso/status minimo/nota minima). Novas funcoes em `src/features/evolucao/api.ts` (`getCategoriasCatalogo`, `criarCategoria`, `atualizarCategoria`, `getHabilidadesCatalogo`, `criarHabilidade`, `atualizarHabilidade`, `getRequisitosNivelAdmin`, `adicionarRequisitoNivel`, `atualizarRequisitoNivel`, `removerRequisitoNivel`) e novo helper `slugify` em `src/lib/texto.ts`. Link novo no `QuickMenu.tsx`, dono-only. Verificado via bundle do Metro (`expo start --web`), sem erro.
- **Escopo reduzido deliberadamente nesta rodada:** edicao inline de nome/descricao de categorias/habilidades ja existentes nao foi implementada (so criar + ativar/desativar) — o achado 8.1 pedia "editar/adicionar/excluir", mas ativar/desativar cobre o "excluir" com seguranca (`habilidades_catalogo` tem `on delete restrict` por causa de `avaliacoes_evolucao`, hard delete quebraria historico) e criar+desativar ja desbloqueia o uso principal (parar de depender de SQL). Editar nome/descricao de item existente fica como complemento pequeno, nao feito agora.
- **Nao feito nesta rodada (fora do "bloqueador"):** pontuacao/score (`evolucao-vs-desempenho.md` §8.2, o proprio documento marca como aditivo, "nao bloqueia nada"); `QuickMenu` com `ScrollView` (achado 6.1 de `papeis-e-permissoes.md`); tela hub "Configuracoes" e as telas de Grade semanal/Tipos de evento (`papeis-e-permissoes.md` §6.3-7, itens 3-6) — ficam registradas como proximo lote.

## 5.2 Retomada (2026-09-20): hub "Configuracoes", Grade semanal, Tipos de evento — proximo lote fechado

Itens 3-6 do backlog de `papeis-e-permissoes.md` §7, mais o achado 6.1 (`QuickMenu` sem scroll, ja fechado antes — ver commit `d8c5051`, feedback direto do usuario testando o app).

- **Grade semanal** (`app/(app)/configuracoes/grade-semanal.tsx`, dono-only) — **fechado.** RLS ja era `papel_atual() = 'dono'` desde `0001` (`grade_escrita`), nenhuma migration nova. CRUD completo: criar horario (dia da semana em chips, hora HH:MM, modulos 1-4 em chips), ativar/desativar sempre disponivel, **excluir so depois de verificar uso** (nova funcao `getUsoSlotGrade`, conta `aulas` e `aulas_teste` vinculadas) — decisao 2 de `papeis-e-permissoes.md` §6.4: excluir sem checar apagaria vinculo historico de chamada (`aulas.aula_recorrente_id on delete set null`) ou cancelaria aula teste em cascata (`aulas_teste on delete cascade`). Novas funcoes em `src/features/chamada/api.ts` (`getGradeCompleta`, `criarSlotGrade`, `atualizarSlotGrade`, `getUsoSlotGrade`, `excluirSlotGrade`).
- **Tipos de evento** (`app/(app)/configuracoes/tipos-evento.tsx`, staff) — **fechado.** RLS ja era `eh_equipe()` desde `0003` (`tipo_evento_escrita`), nenhuma migration nova. Criar (nome + cor via paleta de 8 swatches preset, ordem auto-calculada) + ativar/desativar — sem excluir (mesma logica de "nao vale o risco de FK restrict" do catalogo). Novas funcoes em `src/features/eventos/api.ts` (`getTiposEventoAdmin`, `criarTipoEvento`, `atualizarTipoEvento`).
- **Hub "Configuracoes"** (`app/(app)/configuracoes/index.tsx`) — **fechado.** Lista de cards por papel: dono ve Grade semanal, Horarios por professor, Modulos por professor, Tipos de evento, Usuarios, Catalogo de Evolucao (6 itens); professor ve Meus modulos, Horarios livres, Convidar usuario (3 itens). `QuickMenu.tsx` reduzido: os 5 itens de administracao que estavam soltos (Horarios/Modulos por professor, Usuarios, Catalogo de Evolucao, Convidar usuario) viram 1 icone so ("Configuracoes").
- **Achado no caminho:** `uiAssets.card.perfil` (usado pros itens "Usuarios"/"Convidar usuario" que acabei de consolidar) tinha um comentario dizendo que a imagem era na verdade uma engrenagem "reservada pra uma futura tela de Configuracoes, nao e o icone de perfil" — e o arquivo `assets/cards/card-configuracoes.png` ja existia no repositorio, nunca registrado em `uiAssets.ts`. Corrigido: `card.perfil` (nome confuso, nunca foi usado pela tela de perfil de fato — essa usa `PersonIcon`) removido, `card.configuracoes` aponta pro asset certo.
- **Nao feito, deliberadamente:** pontuacao/score (`evolucao-vs-desempenho.md` §8.2) continua registrado como aditivo puro, sem prioridade — nao foi pedido nesta retomada.

## 5.3 Retomada (2026-09-20): pontuacao/score de Minha Evolucao — ultimo item aditivo fechado

`evolucao-vs-desempenho.md` §8.2, ultimo item da lista das secoes 5.1/5.2 ("nao feito, deliberadamente" nas duas rodadas anteriores por ser aditivo puro, sem bloquear nada). Pedido explicito do PO pra fechar todas as pendencias.

- **Migration `0031_pontuacao_habilidade.sql`** — **fechado.** Coluna nova `habilidades_catalogo.valor_base numeric(6,2) not null default 1` — o "quanto vale" cada habilidade/figura, independente de quem a executa, no mesmo principio das tabelas oficiais de patinacao (BASYS/RollArt). Default 1 (nao 0) pra habilidade recem-criada nao zerar a pontuacao de quem for avaliado nela antes do dono configurar o valor real.
- **`calcularPontuacaoAluno`** (`src/features/evolucao/selectors.ts`) — **fechado.** Pontuacao = soma de `valor_base x fator_qualidade` de cada habilidade exigida no nivel que ja tem avaliacao registrada. `FATOR_QUALIDADE` mapeia os 5 status existentes (nao_iniciado=0 ... consolidado=1), igual ao "confirmado/nao confirmado" do regulamento oficial que o documento de origem cita como equivalente simplificado. **Simplificacao deliberada em relacao ao texto literal do §8.2:** o documento distingue avaliacao rapida (fator por status) de avaliacao detalhada (fator = `percentual_geral / 100`), mas `status_habilidade_aluno` grava `percentual_atual` identicamente nos dois casos (a avaliacao rapida tambem grava um percentual, vindo de um preset fixo por status) — nao ha coluna nem flag que diga qual dos dois caminhos gerou aquele registro. Como o status de uma avaliacao detalhada ja e derivado do percentual real (`derivarStatusPorPercentual`), usar sempre o fator por status e uma aproximacao razoavel sem precisar de join novo so pra essa distincao; documentado como comentario no proprio `selectors.ts`.
- **Catalogo de Evolucao** (`app/(app)/catalogo-evolucao.tsx`) — **fechado.** Aba Habilidades ganhou campo "Valor base" no formulario de criacao e edicao inline (campo numerico por linha, salva no `onEndEditing`) pras habilidades ja cadastradas — igual ao padrao pedido pelo PO ("editavel na mesma tela de catalogo do dono").
